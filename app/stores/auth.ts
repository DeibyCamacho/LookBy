...
export type UserRole = 'admin' | 'profesional' | 'proveedor' | 'cliente' | 'staff' | 'receptionist' | string
...
export const useAuthStore = defineStore('auth', () => {
  ...
  const isAdmin = computed(() => (user.value?.tipoUsuario || user.value?.role) === 'admin')
  const isProfesional = computed(() => (user.value?.tipoUsuario || user.value?.role) === 'profesional')
  const isProveedor = computed(() => (user.value?.tipoUsuario || user.value?.role) === 'proveedor')
  const isCliente = computed(() => (user.value?.tipoUsuario || user.value?.role) === 'cliente')
